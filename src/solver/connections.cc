...
void ConnectionsImpl::update_hamiltonian(const int det_id) {
  ...
  for (auto it = alpha_dets.begin(); it != alpha_dets.end(); it++) {
    ...
    if (det_id >= 57315 && alpha_det_id < 57315 && std::abs(abstract_system->coefs[alpha_det_id] * H) < 0.00005) continue;
    if (det_id < 57315 && alpha_det_id >= 57315 && std::abs(coef * H) < 0.00005) continue;
    //if (det_id >= 57315 && alpha_det_id >= 57315) continue;
    if (det_id >= 57315 && alpha_det_id >= 57315 && std::abs(coef * H) < 0.00005 && 
         std::abs(abstract_system->coefs[alpha_det_id] * H) < 0.00005) continue;
  ...
  for (auto it = beta_dets.begin(); it != beta_dets.end(); it++) {
    ...
    const double H = abstract_system->hamiltonian(&det, &connected_det);
    if (std::abs(H) < std::numeric_limits<double>::epsilon()) continue;
    if (det_id >= 57315 && beta_det_id < 57315 && std::abs(abstract_system->coefs[beta_det_id] * H) < 0.00005) continue;
    if (det_id < 57315 && beta_det_id >= 57315 && std::abs(coef * H) < 0.00005) continue;
    //if (det_id >= 57315 && beta_det_id >= 57315) continue;
    if (det_id >= 57315 && beta_det_id >= 57315 && std::abs(coef * H) < 0.00005 && 
         std::abs(abstract_system->coefs[beta_det_id] * H) < 0.00005) continue;
  ...
  for (const auto alpha_single : alpha_singles) {
    ...
    for (auto it = beta_singles.begin(); it != beta_singles.end(); it++) {
      ...
      if (related_beta_ids[ptr] == beta_single) {
        ...
        const double H = abstract_system->hamiltonian(&det, &connected_det);
        if (std::abs(H) < std::numeric_limits<double>::epsilon()) continue;
        if (det_id >= 57315 && related_det_id < 57315 && std::abs(abstract_system->coefs[related_det_id] * H) < 0.00005) continue;
        if (det_id < 57315 && related_det_id >= 57315 && std::abs(coef * H) < 0.00005) continue;
        //if (det_id >= 57315 && related_det_id >= 57315) continue;
        if (det_id >= 57315 && related_det_id >= 57315 && std::abs(coef * H) < 0.00005 && 
             std::abs(abstract_system->coefs[related_det_id] * H) < 0.00005) continue;
